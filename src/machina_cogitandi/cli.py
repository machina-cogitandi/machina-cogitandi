...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Machina-Cogitandi thinks for you about the meanings.",
        epilog="Example:  machina-cogitandi input_text.txt > output_text.txt"
...
def run():
    """
    $ text | machina-cogitandi                        # Accepts text from the pipe
    $ echo "...<text>..." | machina-cogitandi         #

    $ machina-cogitandi multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="machina-cogitandi",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"machina-cogitandi: {utterance}.")
        syslog.closelog()
